...
let mask_of ~key transformed_data =
  ...
  for i = 0 to Bytes.length transformed_data - 1 do
    let c = (Bytes.get transformed_data i |> int_of_char) lxor Array.get key (i mod 4) in
...
let to_bytes ?masking_key frame =
  ...
  let frame_size =
    1
    + (if payload_length <= 125 then 1 else if payload_length <= 65535 then 3 else 9)
  ...
  Bytes.set
    buf
    0
    (((if frame.fin then 1 else 0) lsl 7) lor int_of_opcode frame.opcode |> char_of_int);
  let mask =
    match masking_key with
    | Some _ -> 1 lsl 7
    | None -> 0
  ...
  (match masking_key with
   ...
   | None -> Bytes.blit frame.payload_data 0 buf !pos payload_length);
  buf
;;
...
let read_frame cin =
  let read_frame cin =
    ...
    let read_bytes offs len =
      if Bytes.length !buf < offs + len then buf := Bytes.extend !buf 0 (offs + len);
    ...
    let payload_data =
      ...
      then (
        ...
        mask_of ~key raw)
      else read_bytes pos payload_length
    in
    { fin; opcode; payload_data }
  in
  try Some (read_frame cin) with
  | _ -> None
;;

let of_bytes bytes =
  let fin = (Bytes.get bytes 0 |> int_of_char) lsr 7 = 1 in
  let opcode = (Bytes.get bytes 0 |> int_of_char) land 15 |> opcode_of_int in
  ...
  let payload_data =
    ...
    then (
      ...
      mask_of ~key raw)
    else Bytes.sub bytes pos payload_length
  in
  { fin; opcode; payload_data }
;;
