...
module Client = struct
  ...
  let umount (_ : Websocket.client) (header : string) : unit =
    Log.info ("Dispatch: A umount message from " ^ header);
    Instance.clients#remove header

  let dispatch (client : Websocket.client) (message : Message.client_message) :
      unit =
    match message with
    | {client_message_header; client_message_service; client_message_body} -> (
      ...
      if Message.is_mount_message (Message.Client (client, message)) then
        mount client client_message_header
      else
        match Instance.find_client client_message_header with
        | None ->
          Log.error ("Dispatch: Client not found : " ^ client_message_header)
        | Some client -> (
...
module Service = struct
  ...
  let dispatch (client : Websocket.client) (message : Message.service_message) :
      unit =
    match message with
    | {service_message_header; service_message_client; service_message_body}
      ...
      if Message.is_mount_message (Message.Service (client, message)) then
        mount client service_message_header
      else
